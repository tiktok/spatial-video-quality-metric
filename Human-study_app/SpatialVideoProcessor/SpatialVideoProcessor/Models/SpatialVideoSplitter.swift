...
extension SpatialVideoSplitter {
    func cleanupTemporaryFiles() {
        ...
        do {
            ...
            for tempFile in tempFiles where tempFile.pathExtension == "mov" {
    ...
    func splitVideo(inputURL: URL) async throws -> (URL, URL) {
        let outputDir = FileManager.default.temporaryDirectory.path
        return try await withCheckedThrowingContinuation { continuation in
            DispatchQueue.global(qos: .userInitiated).async {
                do {
                    self.transcodeMovie(filePath: inputURL.path, outputDir: outputDir)
                    let leftURL = URL(fileURLWithPath: outputDir).appendingPathComponent(inputURL.deletingPathExtension().lastPathComponent + "_LEFT.mov")
                    let rightURL = URL(fileURLWithPath: outputDir).appendingPathComponent(inputURL.deletingPathExtension().lastPathComponent + "_RIGHT.mov")
                    continuation.resume(returning: (leftURL, rightURL))
                } catch {
